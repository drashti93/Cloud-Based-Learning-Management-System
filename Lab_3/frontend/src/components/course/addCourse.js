...
import {addCourseMutation} from "../../mutation/mutation"
...
class AddCourse extends Component {
    ...
    sweetAlert(){
        ...
        swal("Great", "Course added successfull", "success")
        .then(() => {
          this.reload_screen();
        })
    }
    ...
    saveCourse(){
      const result = this.props.addCourseMutation({
        variables: {
            user_id: parseInt(localStorage.getItem('user_id'), 10),
            course_id: parseInt(this.state.course_id, 10),
            course_name: this.state.course_name,
            course_department: this.state.course_department,
            course_description: this.state.course_description,
            room_number: this.state.room_number,
            student_capacity: parseInt(this.state.student_capacity, 10),
            waitlist_capacity: parseInt(this.state.waitlist_capacity, 10),
                    ...
                    <Sidebar/>
                      <div id="assignment_container" className="container">
                        <div>
                            ...
                            <div className="form-row">
                                <button type="submit" className="btn btn-success" name="save_assignment" onClick={() =>{this.saveCourse(); }} >Save</button>
                            </div>
