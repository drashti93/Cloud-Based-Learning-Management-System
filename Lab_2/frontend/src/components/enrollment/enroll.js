...
sweetAlert1(){
    ...
    swal("Great", "Successfully Deopped", "success")
    .then(() => {
      this.reload_screen();
    })
}
...
sweetAlert2(){
    ...
    swal("Great", "Added to waitlist", "success")
    .then(() => {
    //   this.reload_screen();
    })
}
                        ...
                        <tbody>
                          {
                            currentTodos.map((course,index) => {
                               return <tr key={index}>
                                    ...
                                    <td><button className="btn btn-success" hidden={!(course.enrolled === 'no' && course.enrollment_count < course.student_capacity)} onClick= {() => {this.props.enroll(this.props.user_id,course.course_id); this.sweetAlert()}}>Enroll</button></td>
                                    <td><button className="btn btn-danger" hidden={!(course.enrolled === 'yes')} onClick={() => {this.props.drop(this.props.user_id,course.course_id); this.sweetAlert1()}}>Drop</button></td>
                                    <td><button className="btn btn-warning" hidden={!(course.enrolled === 'no' && (course.enrollment_count >= course.student_capacity || course.enrollment_count == course.student_capacity + course.waitlist_capacity))} onClick={() => {this.setState({isEnterPemCodeHidden: false}); this.props.waitlist(this.props.user_id,course.course_id);  this.sweetAlert2()}}>Waitlist</button></td>
                                    <td><button className="btn" onClick={() => swal({text: "Enter code", content: "input"}).then(code => {this.props.getCode(code, this.props.user_id, course.course_id)})}>Enter permission code</button></td>
                                </tr>
