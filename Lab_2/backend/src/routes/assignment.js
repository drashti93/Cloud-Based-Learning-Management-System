...
router.post('/api/uploadfile/', function(req, res){
    ...
    if(req.query.flag == "yes"){
        ...
        Users.update({user_id: user_id, "submissions.assignment_id": id, "submissions.course_id": course_id, "submissions.stype": "assignment"}, {$set: {"submissions.$.isSubmitted": "true"}}).then(() => console.log("set issubmitted"))
        Users.update({user_id: user_id, "submissions.assignment_id": id, "submissions.course_id": course_id, "submissions.stype":"assignment"}, {$push: {"submissions.$.file_name": fileName, "submissions.$.location": "/public"}}, 
        function(err, results){
            if (err) {
                res.status(400);
            }	
